...
9. Khi vẽ biểu đồ bằng matplotlib/seaborn/plotly, BẮT BUỘC tuân thủ NGHIÊM NGẶT PALLETE.md (source of truth cho màu chart) và CHỈ dùng các màu sau cho TOÀN BỘ màu chart: PASTEL_COLORS `#FFBB94`, `#FB9590`, `#DC586D`, `#D44673`, `#B23E59`, `#73293A` (warm peachy-pink series, từ light → dark); ACCENT_COLORS `#FF004C` (hot pink highlight cho outliers/anomalies), `#4D1C2D` (dark burgundy cho text); neutrals `#ffffff` cho nền, `#e9ecef` cho lưới/viền, `#4D1C2D` cho chữ. Code sinh ra PHẢI khai báo hằng `PASTEL_COLORS`, `ACCENT_COLORS`, `BACKGROUND`, `GRID_COLOR`, `TEXT_COLOR` đúng các giá trị này, set `plt.rcParams['axes.prop_cycle']` từ `PASTEL_COLORS`, dùng `facecolor=BACKGROUND`, `ax.set_facecolor(BACKGROUND)`, `grid(color=GRID_COLOR)`, text/tick/label/title màu `TEXT_COLOR`. Với seaborn phải truyền `palette=PASTEL_COLORS` hoặc dict lấy từ danh sách này; với plotly phải truyền `color_discrete_sequence=PASTEL_COLORS`, `color_continuous_scale` tự tạo từ các màu hợp lệ gradient (vd: white → `#73293A`), `paper_bgcolor=BACKGROUND`, `plot_bgcolor=BACKGROUND`, `font_color=TEXT_COLOR`. Không dùng palette mặc định, rainbow, viridis, plasma, magma, inferno, cividis, tab10, Set1/Set2/Set3, Pastel/Pastel1/Pastel2, Dark2, Paired, hoặc bất kỳ tên màu/mã màu nào ngoài danh sách này.
